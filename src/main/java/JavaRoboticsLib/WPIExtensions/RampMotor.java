...
/**
 * A subclass of {@link #SpeedController} intended to allow controlled change in power, for safety
...
public class RampMotor implements SpeedController {
  ...
  public void setMaxAccel(double value) throws Exception {
    if (value < 0) throw new Exception("MaxAccel must be greater than zero. Value given: " + value);
  ...
  public void setMaxDecel(double value) throws Exception {
    if (value < 0) throw new Exception("MaxDecel must be greater than zero. Value given: " + value);
   ...
   * @param port The PWM channel that the motor is attached to. 0-9 are on-board, 10-19 are on the
   *     MXP port
   */
  ...
  /**
   * Wraps an existing {@link #ISpeedController} with {@link #RampMotor{T}}
   *
  ...
  public void set(double value) {
    // The motor is DECELLERATING if |value| < |power| OR value and power
    // are not both positive or both negative
    // Likewise, the motor is ACCELERATING if |value| > |power| AND value
    // and power are both negative or both positive
    // If power is zero, the motor is accelerating.
    // if the motor is ACCELERATING, use MaxAccel. If the robot is
    // DECELLERATING, use MaxDecel.
    boolean accel =
        (Math.signum(value) == Math.signum(m_power) && Math.abs(value) > Math.abs(m_power))
            || Math.abs(m_power) < getMaxAccel();
    double delta = accel ? getMaxAccel() : getMaxDecel();
    if (value > m_power + delta)
      // If the motor wants to change power faster than it is allowed,
      // change it by the max power change allowed
      m_power += delta;
    else if (value < m_power - delta) m_power -= delta;
    else
      // If the motor wants to go to a power within the change
      // limitations, set the power to the value.
      m_power = value;
