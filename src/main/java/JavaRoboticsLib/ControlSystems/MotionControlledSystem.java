...
/**
 * Extendible controller for a {@link #MotionController} Controlled mechanism. Defines No
 * Constructors.
 */
public abstract class MotionControlledSystem {
  ...
  /** Positional tolerance of the {@link #SetPoint} , used in the AtSetPoint property. */
  ...
  public boolean AtSetpoint() {
    return (Sensor.pidGet() < Controller.getSetPoint() + SetpointTolerance)
        && (Sensor.pidGet() > Controller.getSetPoint() - SetpointTolerance);
  }
  ...
  /**
   * Updates the system based on {@link #getSetPoint()}. If the system is in manual, sets the motor
   * to zero
   */
  public void Update() {
    double power = m_manual ? m_manualPower : Controller.get(Sensor.pidGet());

    if ((Sensor.pidGet() < lowLimit && power < 0)
        || (Sensor.pidGet() > highLimit && power > 0) && !m_manual) {
      power = 0;
    }
    Motor.set(power);
  }
