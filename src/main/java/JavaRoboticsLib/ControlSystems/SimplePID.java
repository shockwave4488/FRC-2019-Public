...
public class SimplePID implements MotionController {
  ...
  /**
   * The change in time for this particular PID loop in seconds. Set to negative to determine dt
   * automatically.
   */
  ...
  public SimplePID(double p, double i, double d, double min, double max) throws Exception {
    if (max < min) throw new Exception("Invalid Arguments: {max} Is less than {min}");
