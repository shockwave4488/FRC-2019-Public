...
public class Logger {
  ...
  /**
   * The minimum logger level required to actually log the data - any messages lower leveled than
  ...
  /**
   * Print the {@link #Logger.AddMessage} caller, source file, and line number along with the
   * message
   */
   ...
   *
   * private static boolean _showDetails; public static boolean
   * getShowDetails() { return _showDetails; }
   *
   * public static void setShowDetails(boolean value) { _showDetails = value;
   * }
   */
  ...
  /** Updates the related {@link #SmartDashboard} variable if applicable */
