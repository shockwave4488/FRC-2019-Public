...
import org.usfirst.frc.team4488.lib.util.app.control.SynchronousPIDF;
...
public class SwerveHeadingController {
  ...
  public SwerveHeadingController() {
    if (Constants.kIsUsingTractionWheels) {
      ...
      snapPID = new SynchronousPIDF(0.015, 0.0, 0.00, 0.0);
      stationaryPID = new SynchronousPIDF(0.01, 0.0, 0.002, 0.0);
    } else {
      stabilizationPID = new SynchronousPIDF(0.005, 0.0, 0.0005, 0.0);
      snapPID = new SynchronousPIDF(0.002, 0.0, 0.0, 0.0);
      stationaryPID = new SynchronousPIDF(0.01, 0.0, 0.002, 0.0);
    }
  ...
  public double updateRotationCorrection(double heading, double timestamp) {
    ...
    switch (currentState) {
      ...
      case TemporaryDisable:
        ...
        if (timestamp - disabledTimestamp >= disableTimeLength) setState(State.Stabilize);
        break;
      case Stabilize:
